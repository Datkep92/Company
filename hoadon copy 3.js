...
function createPayableFilterUI() {
    ...
    const filterHtml = `
        <div class="card" id="payable-filter">
            <div class="card-header">
                🔍 Lọc Công Nợ NCC
                <button class="btn btn-sm btn-outline-secondary" onclick="resetPayableFilter()">🔄 Xóa</button>
            </div>
            <div class="card-body">
                <div class="form-grid">
                    <div class="form-group">
                        <input type="text" id="search-payable" placeholder="Tìm kiếm NCC, MST..." class="form-control">
                    </div>
                    <div class="form-group">
                        <select id="debt-filter" class="form-control">
                            <option value="all">Tất cả NCC</option>
                            <option value="debt">Chỉ NCC còn nợ</option>
                            <option value="paid">Chỉ NCC đã trả</option>
                        </select>
                    </div>
                </div>
            </div>
...
function createPayableFilterUI() {
    ...
    header.innerHTML = `
        ...
        <div id="payable-stats" style="margin-top: 8px; font-size: 12px; color: #666;">
            <!-- Thống kê sẽ hiển thị ở đây -->
        </div>
    `;
...
function updatePayableStats(displayed, total) {
    const statsElement = document.getElementById('payable-stats');
    if (statsElement) {
        if (displayed === 0) {
            statsElement.innerHTML = '<span style="color: #dc3545;">❌ Không tìm thấy NCC phù hợp</span>';
        } else {
            statsElement.innerHTML = `📊 Đang hiển thị: <strong>${displayed}</strong>/<strong>${total}</strong> NCC`;
        }
    }
}
// =======================
...
// =======================
// 5. HÀM LOAD MẶC ĐỊNH - SẮP XẾP LỖI TRÊN CÙNG
// =======================
...
function loadPurchaseInvoicesWithDefaultSort() {
    ...
    // SẮP XẾP MẶC ĐỊNH: Lỗi trên cùng, sau đó theo ngày
    invoices.sort((a, b) => {
        const aIsError = a.status && a.status.validation === 'error' && !a.status.stockPosted;
        const bIsError = b.status && b.status.validation === 'error' && !b.status.stockPosted;
...
// =======================
// LỌC HÓA ĐƠN - THÊM LỌC NGÀY
// =======================
// =======================
// SỬA VỊ TRÍ LỌC - ĐẶT ĐÚNG TRÊN MỖI BẢNG
// =======================
...
function createInvoiceFilterUI() {
    ...
    // TÌM ĐÚNG CARD DANH SÁCH HÓA ĐƠN (card thứ 2 trong tab Mua Hàng)
    const invoiceCards = document.querySelectorAll('#mua-hang .content-body .card');
    let invoiceListSection = null;
    ...
    const filterHtml = `
        <div class="card" id="purchase-invoice-filter">
            <div class="card-header">
                🔍 Lọc Hóa Đơn
                <button class="btn btn-sm btn-outline-secondary" onclick="resetInvoiceFilter()">🔄 Xóa</button>
            </div>
            <div class="card-body">
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px;">
                    <div class="form-group">
                        <label>Tìm kiếm</label>
                        <input type="text" id="search-invoices" placeholder="Tên NCC, MST, Số HĐ..." class="form-control">
                    </div>
                    <div class="form-group">
                        <label>Khoảng thời gian</label>
                        <select id="date-filter-invoices" class="form-control">
    ...
    // CHÈN NGAY TRƯỚC CARD DANH SÁCH HÓA ĐƠN
    invoiceListSection.insertAdjacentHTML('beforebegin', filterHtml);
}
...
function applyInvoiceFilters() {
    ...
    if (searchTerm) {
        const searchTerms = searchTerm.split(' ').filter(term => term.length > 0);
        if (searchTerms.length > 0) {
            invoices = invoices.filter(invoice => searchTerms.every(term => 
                invoice.invoiceInfo.symbol.toLowerCase().includes(term) ||
                invoice.invoiceInfo.number.toLowerCase().includes(term) ||
                invoice.sellerInfo.taxCode.toLowerCase().includes(term) ||
                invoice.sellerInfo.name.toLowerCase().includes(term)
...
// =======================
// HÀM LỌC CÔNG NỢ - THÊM LỌC TRẠNG THÁI
// =======================
...
function applyPayableFilters() {
    ...
    if (searchTerm) {
        const searchTerms = searchTerm.split(' ').filter(term => term.length > 0);
        if (searchTerms.length > 0) {
            suppliers = suppliers.filter(supplier => searchTerms.every(term => 
                supplier.name.toLowerCase().includes(term) ||
                supplier.taxCode.toLowerCase().includes(term)
            ));
        }
